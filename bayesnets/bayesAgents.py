...
def constructBayesNet(gameState):
    ...
    for house_var in HOUSE_VARS:
        edges.append((X_POS_VAR, house_var))
        edges.append((Y_POS_VAR, house_var))
        for obs_var in obsVars:
            edges.append((house_var, obs_var))
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    f = inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    assignments = f.getAllPossibleAssignmentDicts()

    probs = [f.getProbability(assignment) for assignment in assignments]
    
    max_prob = max(probs)
    for assignment in assignments:
        if f.getProbability(assignment) == max_prob: return assignment
    "*** END YOUR CODE HERE ***"
