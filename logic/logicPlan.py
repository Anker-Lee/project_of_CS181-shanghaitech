...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combinationsRes = list(itertools.combinations(literals, 2))
    res = []
    for combination in combinationsRes:
        res.append(disjoin([~expr for expr in combination]))
    return conjoin(res)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Current <==> (previous position at time t-1) & (took action to move to x, y)
    CurrentPos = PropSymbolExpr(pacman_str, x, y, time=time)
    return CurrentPos % atLeastOne(possible_causes)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in all_coords:
        pacphysics_sentences += [PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=t)]
    
    Possible_poses = []
    for x, y in non_outer_wall_coords:
        Possible_poses += [PropSymbolExpr(pacman_str, x, y, time=t)]
    pacphysics_sentences += [exactlyOne(Possible_poses)]

    Possible_dirs = []
    for direct in DIRECTIONS:
        Possible_dirs.append(PropSymbolExpr(direct, time=t))
    pacphysics_sentences.append(exactlyOne(Possible_dirs))
    
    if sensorModel != None:
        Sensor_res = sensorModel(t, non_outer_wall_coords)
        pacphysics_sentences += [Sensor_res]

    if successorAxioms != None:
        if t == 0: pass
        else:
            Successor_res = successorAxioms(t, walls_grid, non_outer_wall_coords)
            pacphysics_sentences += [Successor_res]
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    given_info = [pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid)]
    given_info += [pacphysicsAxioms(0+1, all_coords, non_outer_wall_coords, walls_grid, successorAxioms=allLegalSuccessorAxioms)]
    given_info += [PropSymbolExpr(pacman_str, x0, y0, time=0)]
    given_info += [PropSymbolExpr(action0, time=0)]
    given_info += [PropSymbolExpr(action1, time=1)]

    KB.append(conjoin(given_info))

    cause = PropSymbolExpr(pacman_str, x1, y1, time=1)
    causeInverse = ~PropSymbolExpr(pacman_str, x1, y1, time=1)
    KB_cause = KB + [cause]
    KB_notCause = KB + [causeInverse]
    model_1 = findModel(conjoin(KB_cause))
    model_2 = findModel(conjoin(KB_notCause))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # check is goal
        goal_maybe = conjoin(KB + [PropSymbolExpr(pacman_str, xg, yg, time=t)]) & PropSymbolExpr(pacman_str, xg, yg, time=t)
        satisfied_model = findModel(goal_maybe)
        if satisfied_model != False:
            return extractActionSequence(satisfied_model, actions)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        goal_maybe = conjoin(KB + food_eaten)
        satisfied_model = findModel(goal_maybe)
        if satisfied_model != False:
            return extractActionSequence(satisfied_model, actions)
        ...
        # food successor
        for fx, fy in food:
            pacmanIsHere = PropSymbolExpr(pacman_str, fx, fy, time=t)
            hasFoodNow = PropSymbolExpr(food_str, fx, fy, time=t)
            hasFoodNext = PropSymbolExpr(food_str, fx, fy, time=t+1)
            # 本身没有食物 or 满足吃掉条件
            KB.append( ~hasFoodNow | ~hasFoodNext % (pacmanIsHere & hasFoodNow) )   
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # where the walls are (walls_list) and aren't (not in walls_list).
    non_wall_coords = [loc for loc in all_coords if loc not in walls_list]
    KB += [PropSymbolExpr(wall_str, x, y) for x,y in walls_list]
    KB += [~PropSymbolExpr(wall_str, x, y) for x,y in non_wall_coords]

    for t in range(agent.num_timesteps):
        action = agent.actions[t]
        # Add pacphysics, action, and percept information to KB
        # pacphysics.
        KB += [pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sensorModel=sensorAxioms, successorAxioms=allLegalSuccessorAxioms)]
        # action
        KB += [PropSymbolExpr(action, time=t)]
        # percept information
        ...
        for x, y in non_outer_wall_coords:
            P_x_y_t_1 = PropSymbolExpr(pacman_str, x, y, time=t)
            P_x_y_t_0 = ~P_x_y_t_1
            pac_at_x_y = entails(conjoin(KB), P_x_y_t_1)
            pac_not_at_x_y = entails(conjoin(KB), P_x_y_t_0)
            if pac_at_x_y: KB += [P_x_y_t_1]
            if pac_not_at_x_y: KB += [P_x_y_t_0]
            else: possible_locations += [(x, y)]
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        # Add pacphysics, action, and percept information to KB.
        # pacphysics.
        KB += [pacphysicsAxioms(t, all_coords, non_outer_wall_coords, known_map, sensorModel=sensorAxioms, successorAxioms=allLegalSuccessorAxioms)]
        # action
        KB += [PropSymbolExpr(action_t, time=t)]
        # percept information
        percepts = agent.getPercepts()
        percept_rules = fourBitPerceptRules(t=t, percepts=percepts)
        KB += [percept_rules]
        ...
        for x, y in non_outer_wall_coords:
            w_x_y_1 = PropSymbolExpr(wall_str, x, y)
            w_x_y_0 = ~w_x_y_1
            pac_at_x_y = entails(conjoin(KB), w_x_y_1)
            pac_not_at_x_y = entails(conjoin(KB), w_x_y_0)
            if pac_at_x_y: 
                KB += [w_x_y_1]
                known_map[x][y] = 1
            elif pac_not_at_x_y: 
...
def slam(problem, agent) -> Generator:
    ...
    # pacman pos.
    KB += [PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0)]
    KB += [~PropSymbolExpr(wall_str, pac_x_0, pac_y_0)]
    ...
    for t in range(agent.num_timesteps):
        ...
        # Add pacphysics, action, and percept information to KB.
        # pacphysics.
        KB += [pacphysicsAxioms(t, all_coords, non_outer_wall_coords, known_map, sensorModel=SLAMSensorAxioms, successorAxioms=SLAMSuccessorAxioms)]
        # action
        KB += [PropSymbolExpr(action_t, time=t)]
        # percept information
        percepts = agent.getPercepts()
